...
/**
 * ShadowBorder is designed to be immutable, so that it can safely cache the
 * "shadow fragments" images, and reuse them for efficiency.
 * 
 * <p/>
 * The shadow fragments images are determined by the size, color, and alpha of
 * the shadow.
 * 
 * <p/>
 * Instance can be retrieved via an overloading {@code newInstance()} method or
 * a {@code ShadowBorder.ObjectFactory} instance, using a "builder design
 * pattern".
 * 
 * <p/>
...
public class ShadowBorder implements Border {
	...
	private BufferedImage[] getShadowPieces() {
		...
		if (shadows == null) {
			...
			BufferedImage shadow = new BufferedImage(bufferBounds.width, bufferBounds.height,
