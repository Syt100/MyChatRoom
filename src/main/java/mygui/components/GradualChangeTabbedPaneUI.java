...
/**
 * 自定义TabbedPaneUI，渐变标签
 * 
 * @author https://blog.csdn.net/yuanzihui/article/details/43936795
 *
 */
public class GradualChangeTabbedPaneUI extends BasicTabbedPaneUI {
	...
	private ColorSet hoverColorSet;// 悬停颜色
	
	private int lastRollOverTab = -1;// 上一次滚动标签
	private RollOverListener l;

	public GradualChangeTabbedPaneUI(String bgColorHex, String frontColorHex) {
	...
	public static ComponentUI createUI(JComponent c) {
		return new GradualChangeTabbedPaneUI("#FFFFFF", "#000000");
	}

	@Override
	protected void installDefaults() {
		super.installDefaults();
		l = new RollOverListener();
	...
	protected int calculateTabWidth(int tabPlacement, int tabIndex, FontMetrics metrics) {
		// return 120;// 这个才是固定
		return 10 + anchoFocoV + super.calculateTabWidth(tabPlacement, tabIndex, metrics);
	}
...
//      if (isSelected) {
		...
		int[] xp = new int[] { x, x, x + 3, x + w - inclTab - 6, x + w - inclTab - 3, x + w - inclTab, x + w - inclTab,
				x };
...
//			 g.drawPolygon(shape);
	...
	private class RollOverListener extends MouseAdapter {
		...
		private void checkRollOver() {
			...
			if (currentRollOver != lastRollOverTab) {
				...
				Rectangle tabsRect = new Rectangle(0, 0, tabPane.getWidth(), tabPane.getHeight() + 1);
	...
	protected void uninstallListeners() {
		...
		if (l != null) {
			...
			tabPane.removeMouseMotionListener(l);
			l = null;
		}
	}
}